        ...
        (function (global) {
          ...
          var FLACDecoder = AV.Decoder.extend(function () {
            ...
            const SAMPLE_RATES = new Int32Array([
              0, 88200, 176400, 192000, 8000, 16000, 22050, 24000, 32000, 44100,
              48000, 96000, 0, 0, 0, 0,
            ]);
            ...
            this.prototype.readChunk = function () {
              ...
              if (channels !== this.format.channelsPerFrame)
                throw new Error(
                  "Switching channel layout mid-stream not supported."
                );
              ...
              if (this.bps !== this.format.bitsPerChannel)
                throw new Error(
                  "Switching bits per sample mid-stream not supported."
                );
              ...
              for (; ones > 1; ones--) {
                stream.advance(2); // == 2
                frame_or_sample_num =
                  (frame_or_sample_num << 6) | stream.read(6);
              }
              ...
              var is32 = this.bps > 16,
                output = new ArrayBuffer(
                  this.blockSize * channels * (is32 ? 4 : 2)
                ),
            ...
            this.prototype.decodeSubframe = function (channel) {
              ...
              if (channel === 0) {
                ...
                if (
                  this.chMode === CHMODE_LEFT_SIDE ||
                  this.chMode === CHMODE_MID_SIDE
                )
                  this.curr_bps++;
              }
              ...
              if (this.curr_bps > 32)
                throw new Error(
                  "decorrelated bit depth > 32 (" + this.curr_bps + ")"
                );
            ...
            this.prototype.decode_subframe_fixed = function (
              channel,
              predictor_order
            ) {
              ...
              if (predictor_order > 2)
                c =
                  b -
                  decoded[predictor_order - 2] +
                  decoded[predictor_order - 3];

              if (predictor_order > 3)
                d =
                  c -
                  decoded[predictor_order - 2] +
                  2 * decoded[predictor_order - 3] -
                  decoded[predictor_order - 4];
            ...
            this.prototype.decode_subframe_lpc = function (
              channel,
              predictor_order
            ) {
              var stream = this.bitstream,
              ...
              if (this.bps <= 16) {
                ...
                for (var i = predictor_order; i < blockSize; i++) {
                  ...
                  decoded[i] +=
                    (total[0] >>> qlevel) | (total[1] << (32 - qlevel));
                }
              }
            };
            ...
            this.prototype.decode_residuals = function (
              channel,
              predictor_order
            ) {
              var stream = this.bitstream,
              ...
              if (method_type > 1)
                throw new Error(
                  "Illegal residual coding method " + method_type
                );
              ...
              if (predictor_order > samples)
                throw new Error(
                  "Invalid predictor order " + predictor_order + " > " + samples
                );
              ...
              while (stream.available(1) && !this.last) {
                ...
                switch (this.type) {
                  case STREAMINFO:
                    ...
                    this.emit(
                      "duration",
                      ((sampleCount / this.format.sampleRate) * 1000) | 0
                    );
                  ...
                  case SEEKTABLE:
                    for (var s = 0; s < this.size / 18; s++) {
                      if (
                        stream.peekUInt32(0) == 0xffffffff &&
                        stream.peekUInt32(1) == 0xffffffff
                      ) {
                        //placeholder, ignore
                        stream.advance(18);
                      } else {
                        if (stream.readUInt32() > 0) {
                          this.emit(
                            "error",
                            "Seek points with sample number >UInt32 not supported"
                          );
                        }
                        ...
                        if (stream.readUInt32() > 0) {
                          this.emit(
                            "error",
                            "Seek points with stream offset >UInt32 not supported"
                          );
                        }
                  ...
                  case VORBIS_COMMENT:
                    ...
                    for (var i = 0; i < length; i++) {
                      ...
                      this.metadata[str.slice(0, idx).toLowerCase()] =
                        str.slice(idx + 1);
                    }
      ...
      function (require, module, exports) {
        (function (global) {
          ...
          try {
            var OggDemuxer =
              typeof window !== "undefined"
                ? window["AV"]["OggDemuxer"]
                : typeof global !== "undefined"
                ? global["AV"]["OggDemuxer"]
                : null;
          } catch (e) {}
          ...
          OggDemuxer.plugins.push({
            ...
            readHeaders: function (packet) {
              ...
              this.flac = AV.Demuxer.find(
                stream.peekSingleBuffer(0, stream.remainingBytes())
              );
