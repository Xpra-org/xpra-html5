...
class XpraPaintWorker {
    ...
    update_still(wid) {
        let canvas = this.offscreen_canvas.get(wid);
        let still = this.offscreen_canvas_still.get(wid);
        still.getContext("2d").drawImage(canvas, 0, 0, canvas.width, canvas.height, 0, 0, canvas.width, canvas.height);
    }
    ...
    redraw(wid) {
        if (KEEP_STILLS) {
            let canvas = this.offscreen_canvas.get(wid);
            let still = this.offscreen_canvas_still.get(wid);
            canvas.getContext("2d").drawImage(still, 0, 0, still.width, still.height, 0, 0, still.width, still.height);
        } else {
            console.warn(`PaintWorker was asked for a redraw on window ${wid} but no still available!`);
        }
    }
